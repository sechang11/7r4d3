//+------------------------------------------------------------------+
//|                                                  RiskManager.mq4 |
//|              MQL4 port of RiskManager.mq5 (originally MQL5 v6.00)|
...
// [MQL4 PORT] Original MQL5 includes commented out; replaced by the
// inline compatibility layer below. MQL4 (build 600+) has WebRequest,
// MqlRates, MqlDateTime, SymbolInfoDouble, AccountInfoDouble, classes,
// ulong, EventSetMillisecondTimer, etc. natively, so most code ports 1:1.
// The differences handled by the shim:
//   1. MQL5 has separate "positions" and "orders". MQL4 has a single
//      orders pool. Shim: PositionsTotal/PositionGet* iterate filtered.
//   2. MQL5's CTrade class. Shim: minimal CTrade wrapper around OrderSend.
//   3. MQL5 enum constants (POSITION_*, ORDER_*) defined as #defines.
//   4. MQL5's TimeCurrent(dt&) overload not in MQL4 -> rewritten as
//      TimeToStruct(TimeCurrent(), dt) (already substituted in source).
// ------------------------------------------------------------------

// =================== MQL4 COMPATIBILITY SHIM ======================

// --- MQL5 enum value mappings (POSITION_* / ORDER_*) ---
#define POSITION_TYPE_BUY      0   // matches OP_BUY
#define POSITION_TYPE_SELL     1   // matches OP_SELL
#define POSITION_SYMBOL        101
#define POSITION_TYPE          102
#define POSITION_VOLUME        103
#define POSITION_PRICE_OPEN    104
#define POSITION_SL            105
...
#define ORDER_PRICE_OPEN       203
#define ORDER_SL               204
#define ORDER_TP               205
#define ORDER_TIME_GTC         0    // unused by MQL4 OrderSend, kept for sig compat

// --- Position iteration shims (MQL5 PositionsTotal/PositionGet*) ---
// Note: These wrap MQL4 OrderSelect with type filter. They do NOT
// modify the MQL4 selection across calls in a way other code expects;
// each PositionGet*/OrderGet* call must follow PositionGetTicket(i)
// in the same loop iteration, exactly as in the MQL5 idiom.
...
ulong PositionGetTicket(int idx)
{
   int total = OrdersTotal();
   int count = 0;
   for(int i = 0; i < total; i++)
   {
      ...
      if(count == idx) return (ulong)OrderTicket();
      count++;
   }
   return 0;
}

string PositionGetString(int prop)
{
   if(prop == POSITION_SYMBOL) return OrderSymbol();
   return "";
}

double PositionGetDouble(int prop)
{
   if(prop == POSITION_VOLUME)     return OrderLots();
   if(prop == POSITION_PRICE_OPEN) return OrderOpenPrice();
   if(prop == POSITION_SL)         return OrderStopLoss();
   if(prop == POSITION_TP)         return OrderTakeProfit();
   if(prop == POSITION_PROFIT)     return OrderProfit();
   if(prop == POSITION_SWAP)       return OrderSwap();
   return 0;
}

// MQL4 has no GetTickCount64(). GetTickCount() is 32-bit and wraps about
// every 49 days; count the wraps so a queued split leg cannot end up waiting
// weeks if one happens mid-send.
ulong GetTickCount64()
{
   static uint  lastTick = 0;
   static ulong wrapBase = 0;
   uint now = GetTickCount();
   if(now < lastTick) wrapBase += 4294967296;   // 2^32
   lastTick = now;
   return wrapBase + (ulong)now;
}
...
long PositionGetInteger(int prop)
{
   ...
   {
      int t = OrderType();
      if(t == OP_BUY)  return POSITION_TYPE_BUY;
      if(t == OP_SELL) return POSITION_TYPE_SELL;
   }
   return 0;
}

// --- Pending-order iteration shims (MQL5 OrderGetTicket/OrderGet*) ---
// MQL5's OrdersTotal returns count of pending orders only. MQL4's
// OrdersTotal returns count of all orders (pending + open). The
// callers in this EA all use the safety pattern
//   ulong ticket = OrderGetTicket(i); if(ticket == 0) continue;
// so OrderGetTicket below returns 0 for non-pending indices, letting
// the loop self-filter. PendingOrdersTotal is unused but kept as a
// reference for future code that wants tight loop bounds.
...
ulong OrderGetTicket(int idx)
{
   int total = OrdersTotal();
   if(idx < 0 || idx >= total) return 0;
   if(!OrderSelect(idx, SELECT_BY_POS, MODE_TRADES)) return 0;
   if(OrderType() <= OP_SELL) return 0;   // open position, not pending
   return (ulong)OrderTicket();
}

string OrderGetString(int prop)
{
   if(prop == ORDER_SYMBOL) return OrderSymbol();
   return "";
}

double OrderGetDouble(int prop)
{
   if(prop == ORDER_VOLUME_CURRENT) return OrderLots();
   if(prop == ORDER_PRICE_OPEN)     return OrderOpenPrice();
   if(prop == ORDER_SL)             return OrderStopLoss();
   if(prop == ORDER_TP)             return OrderTakeProfit();
   return 0;
}

// --- CTrade compatibility class (wraps MQL4 OrderSend/Modify/Close) ---
class CTrade
{
public:
   int   m_magic;
   int   m_slippage;
                ...
                CTrade()                       { m_magic = 0; m_slippage = 10; }
   void         SetExpertMagicNumber(int m)    { m_magic = m; }
   void         SetDeviationInPoints(int s)    { m_slippage = s; }

   bool Buy(double lots, string sym, double price, double sl, double tp, string comment)
   {
      double p = (price <= 0) ? MarketInfo(sym, MODE_ASK) : price;
      int t = OrderSend(sym, OP_BUY, lots, p, m_slippage, sl, tp, comment, m_magic, 0, clrBlue);
      return (t > 0);
   }
   bool Sell(double lots, string sym, double price, double sl, double tp, string comment)
   {
      double p = (price <= 0) ? MarketInfo(sym, MODE_BID) : price;
      int t = OrderSend(sym, OP_SELL, lots, p, m_slippage, sl, tp, comment, m_magic, 0, clrRed);
      return (t > 0);
   }
   bool BuyLimit(double lots, double price, string sym, double sl, double tp, int timeMode, datetime expiry, string comment)
   {
      int t = OrderSend(sym, OP_BUYLIMIT, lots, price, m_slippage, sl, tp, comment, m_magic, expiry, clrBlue);
      return (t > 0);
   }
   bool SellLimit(double lots, double price, string sym, double sl, double tp, int timeMode, datetime expiry, string comment)
   {
      int t = OrderSend(sym, OP_SELLLIMIT, lots, price, m_slippage, sl, tp, comment, m_magic, expiry, clrRed);
      return (t > 0);
   }
   bool BuyStop(double lots, double price, string sym, double sl, double tp, int timeMode, datetime expiry, string comment)
   {
      int t = OrderSend(sym, OP_BUYSTOP, lots, price, m_slippage, sl, tp, comment, m_magic, expiry, clrBlue);
      return (t > 0);
   }
   bool SellStop(double lots, double price, string sym, double sl, double tp, int timeMode, datetime expiry, string comment)
   {
      int t = OrderSend(sym, OP_SELLSTOP, lots, price, m_slippage, sl, tp, comment, m_magic, expiry, clrRed);
      return (t > 0);
   }
   bool PositionClose(ulong ticket)
   {
      ...
      if(type > OP_SELL)
      {
         // Pending: cancel via OrderDelete instead
         return ::OrderDelete(tk);
      }
      double lots = OrderLots();
      double price = (type == OP_BUY) ? MarketInfo(sym, MODE_BID) : MarketInfo(sym, MODE_ASK);
      return ::OrderClose(tk, lots, price, m_slippage, clrViolet);
   }
   bool PositionClosePartial(ulong ticket, double lots)
   {
      int tk = (int)ticket;
      if(!OrderSelect(tk, SELECT_BY_TICKET)) return false;
      int type = OrderType();
      if(type > OP_SELL) return false;
      string sym = OrderSymbol();
      double price = (type == OP_BUY) ? MarketInfo(sym, MODE_BID) : MarketInfo(sym, MODE_ASK);
      return ::OrderClose(tk, lots, price, m_slippage, clrViolet);
   }
   bool PositionModify(ulong ticket, double sl, double tp)
   {
      ...
      return ::OrderModify(tk, openPrice, sl, tp, expiry, clrYellow);
   }
...
// RM_VERSION is stamped into every state POST so the web app can detect
// a stale EA and refuse to present its data as live.
...
// and refuses entries it forbids. Set BOTH to false to disable entirely
// (the documented escape hatch — enforcement must never trap you).
...
// Armed orders re-run on (a) M15 bar change and (b) every 3 seconds intra-bar
// (3-sec throttle gives the user time to drag lines before they snap to price).
int      g_armedOrderLastBars   = 0;
string   g_armedOrderTrackedBtn = "";
datetime g_armedOrderLastTime   = 0;
...
bool     g_tt_lastContBosIsHigh  = false;
...
void BuildAlertsPanel(int leftEdge, int topY, int totalH)
{
   ...
   ObjectSetString(0, "RM_BtnAltCTR", OBJPROP_TOOLTIP,
      ...
      "\n"
      "Click to toggle the alert. ON by default.");
   cy += apBtnH + apGap;
...
//| BUY pattern: candle closes above last down candle's open.        |
//| SELL pattern: candle closes below last up candle's open.         |
//| cstrend (1=up/2=down) flips down when close < last buy-pattern's |
//| down-candle close, up when close > last sell-pattern's up close. |
//| Each timeframe needs its own CTrendState.                        |
//+==================================================================+
struct CTrendState
{
   bool     initialized;
...
void CTrendProcessBar(CTrendState &s, double openx, double highx, double lowx, double closex)
{
   ...
   if(s.cstrend == 1 && closex < s.last_dn_pattern_c)       s.cstrend = 2;
   else if(s.cstrend == 2 && closex > s.last_up_pattern_c)  s.cstrend = 1;
}

//+------------------------------------------------------------------+
//| Paint a filled red rectangle over one M5 candle. De-dupes.       |
//+------------------------------------------------------------------+
void DrawCTrendFlipMarker(datetime barTime, double bh, double bl)
{
   ...
   ObjectSetInteger(0, nm, OBJPROP_WIDTH, 1);            // no distinct border
   ObjectSetInteger(0, nm, OBJPROP_SELECTABLE, false);
   ObjectSetInteger(0, nm, OBJPROP_ZORDER, 0);
}

void CTrendUpdate(int tf, CTrendState &s, int backfillBars, datetime markFlipsFrom = 0)
{
...
void CheckCTrendAlert()
{
   ...
   if(g_alert_ctrLastTrend == 0) { g_alert_ctrLastTrend = t; return; }
...
//+------------------------------------------------------------------+
//| Standard alert-message format:                                   |
//|     "<emoji> <TAG> | <Spoken Symbol> | <event>"                  |
//| No raw price levels, so the spoken alert stays short and clean.  |
//+------------------------------------------------------------------+
...
void ToggleAlertCHCH()
{
   ...
   if(g_alertCHCH)
   {
      ...
      // Dedup on pivot TIME (not price) so we don't re-fire when bar 0's
      // intra-bar low/high makes swingLow/swingHigh drift tick-to-tick.
      if(g_tt_tTrend == 1)
...
void CheckCHCHAlert()
{
   ...
   // Dedup on swingLowTime — pivot bar time is stable within an M15 bar even
   // when ComputeThrust re-runs on breach ticks and re-derives swingLow from
   // bar 0's intra-bar low (which used to cause repeat alerts).
   if(g_tt_tTrend == 1)
   {
...
void PlotDailyMtx()
{
   ...
   for(int d = 0; d < copied; d++)
   {
      ...
      // [MQL4 PORT] strict mode rejects non-constant initializers; assign per-element
      double levels[8];
      levels[0] = prevL - range / 2.0;             // -50
      levels[1] = prevL - range / 4.0;             // -25
      levels[2] = prevL + range / 4.0;             //  25
      levels[3] = prevL + range / 3.0;             //  33
      levels[4] = prevL + 2.0 * range / 3.0;       //  66
      levels[5] = prevL + 3.0 * range / 4.0;       //  75
      levels[6] = prevH + range / 4.0;             // 125
      levels[7] = prevH + range / 2.0;             // 150
...
void PlotWeeklyMtx()
{
   ...
   for(int w = 0; w < copied; w++)
   {
      ...
      // [MQL4 PORT] strict mode rejects non-constant initializers
      double levels[8];
      levels[0] = prevL - range / 2.0;
      levels[1] = prevL - range / 4.0;
      levels[2] = prevL + range / 4.0;
      levels[3] = prevL + range / 3.0;
      levels[4] = prevL + 2.0 * range / 3.0;
      levels[5] = prevL + 3.0 * range / 4.0;
      levels[6] = prevH + range / 4.0;
      levels[7] = prevH + range / 2.0;
      string labels[] = {"-50", "-25", "25", "33", "66", "75", "125", "150"};
...
void PlotTTDRange()
{
   ...
   datetime lblTime = t0 + 3 * PeriodSeconds(PERIOD_M15);
...
//+------------------------------------------------------------------+
//| Scan M15 bars from fromTime to bar-2 for the deepest unfilled    |
//| 3-candle wick FVG in the given direction.                        |
//| direction = +1 bullish (c2.high < c0.low), entry = c2.high.      |
//|            -1 bearish (c2.low  > c0.high), entry = c2.low.       |
//| swingFilter: bullish FVG bottom must be > filter; bearish < filt.|
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| CHOCH Retrace limit order (wick-FVG variant).                    |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| BOS Retrace limit order (wick-FVG variant).                      |
//+------------------------------------------------------------------+
...
//|   range = (highest high since last swing low) − swing low.      |
//| +UFV (buy):  downtrend, ask < swing low. SL = entry − range,    |
//|   range = swing high − (lowest low since last swing high).      |
//+------------------------------------------------------------------+
...
int BridgeMode()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(OrderType() == OP_BUY || OrderType() == OP_SELL) return BRIDGE_ACTIVE;
   }
...
bool DispatchRemote(string action, string button, string body, string &note)
{
   ...
   if(action == "setSL" || action == "setTP")
   {
      ...
      for(int i = OrdersTotal() - 1; i >= 0; i--)
      {
         ...
         double sl = (action == "setSL") ? px : OrderStopLoss();
         double tp = (action == "setTP") ? px : OrderTakeProfit();
         if(g_trade.PositionModify((ulong)OrderTicket(), sl, tp)) n++;
      }
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam)
{
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      ...
      // ── New bet functions (CH_R, BS_R, CH_C, UFV) ──
      if(sparam == "RM_BuyLmtChR" || sparam == "RM_SellLmtChR")
      {
